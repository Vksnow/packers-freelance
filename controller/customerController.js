...
export const GetCustomerController = async (req, res, next) => {
    try {
        ...
        const customer_data = await GetCustomerService(company_id,data.limit,data.page,data.search_item,staff_id)
...
export const updateCustomerController = async (req, res, next) => {
    try {
        ...
        const allowedField = [
            ...
            "mt_state","mt_pincode", "mt_address", "mt_floor", "mt_lift_status", "party_company", "party_company_gst","moving_type","email","reminder_status"
        ];
