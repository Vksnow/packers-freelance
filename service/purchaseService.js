...
export const AddPurchaseService = async (data, Purchase_id) => {
  const insertQuery = `INSERT INTO staff_purchase_plan (purchase_id,staff_count,price ) values (?,?,?)`;
  const values = [Purchase_id,data.staff_count,data.price ];
  try {
    const [inserted_data] = await pool.query(insertQuery, values)
    if (inserted_data.affectedRows === 0) {
      return { success: false, message: "purchase failed to insert", data: inserted_data }
    }
...
export const GetPurchaseService = async () => {
  const quotationQuery = `select  * from staff_purchase_plan where active = ?`;
  try {
...
export const updatePurchaseService = async ( fields, values, Purchase_id) => {
  const setClause = fields.map((field, i) => `${field} = ? `)
  const final_values = [...values, Purchase_id]
  const updateQuery = `update  staff_purchase_plan set  ${setClause} where  Purchase_id =?`;
  ...
  try {
    ...
    if (update_data.affectedRows === 0) {
      return { success: false, message: "Purchase Id is Not Available on Database", data: update_data }
    }
